...
from app.schemas.usuario import UsuarioCreate, UsuarioUpdate, UsuarioOut, UsuarioFCMToken
...
# -------------------------------------------------------
# PATCH /usuarios/fcm-token
# PRIVADO - Cualquier usuario autenticado
# Actualiza el token FCM del usuario logueado
# -------------------------------------------------------
@router.patch("/fcm-token", status_code=status.HTTP_200_OK, tags=["CU4 - Gestión de Perfil de Usuario"])
def actualizar_fcm_token(
    datos: UsuarioFCMToken,
    db: Session = Depends(get_db),
    current_user: Usuario = Depends(get_current_user)
):
    usuario = db.query(Usuario).filter(Usuario.id_usuario == current_user.id_usuario).first()
