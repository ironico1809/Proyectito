...
import base64
from app.routers.incidentes import transcribir_audio_groq

class AudioTranscripcionRequest(BaseModel):
    audio_base64: str

router = APIRouter(prefix="/ia", tags=["CU8 - Procesamiento Multimodal (IA)"])

@router.post("/transcribir")
def transcribir_audio(request: AudioTranscripcionRequest):
    try:
        audio_bytes = base64.b64decode(request.audio_base64)
        texto = transcribir_audio_groq(audio_bytes)
        return {"texto": texto}
    except Exception as e:
        return {"texto": f"Error de transcripción: {str(e)}"}
