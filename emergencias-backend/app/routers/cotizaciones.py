...
async def aceptar_cotizacion(
    ...
    # 4. CU21 — registrar cotización aceptada en bitácora
    registrar_evento(
        db, cotizacion.incidente_id,
        "COTIZACION_ACEPTADA",
    ...
    # 5. Broadcast WebSocket — notificar a todos en la sala del incidente
    await gestor.broadcast(incidente.id_incidente, {
        "tipo": "cambio_estado",
        "estado": "taller_asignado",
        "mensaje": "El cliente aceptó una cotización. Taller asignado.",
        "taller_id": cotizacion.taller_id,
        "timestamp": datetime.now().isoformat()
    })

    return {"mensaje": "Cotización aceptada. Taller asignado al incidente."}
